...
from users.serializers import (EditUserSerializer, ReadUserSerializer,
                               UserSerializer)
...
class AdminViewSet(viewsets.GenericViewSet,mixins.CreateModelMixin,mixins.RetrieveModelMixin,mixins.UpdateModelMixin,mixins.ListModelMixin,mixins.DestroyModelMixin):
    ...
    def change_password(self, request, pk):
        ...
        if len(new_password) < 8:
            ...
            return Response({'message':"Password Matches Email Try another"},status=status.HTTP_400_BAD_REQUEST)
        else:
        ...
        if serializer.is_valid():
            try:
                ...
                if get_user_model().objects.filter(email=user_email).exists():
                    return Response({'message':f'User with {user_email} Email already Exists'},status=200)
                return Response({'message': f'User with {user_ph} Phone Number already Exists'}, status=200)
    ...
    def destroy(self,request, *args, **kwargs):
        try :
            ...
            if self.get_object().id == request.user.id:
                return Response({"message":"Cant Delete yourself "},status=status.HTTP_400_BAD_REQUEST)
            self.perform_destroy(self.get_object())
            logger.warning(f'{user} Deleted {deleted_user}')
            return Response({"message":"Admin Deleted"},status=status.HTTP_204_NO_CONTENT)
        except:
            return Response({"message":"Admin Already Deleted"},status=status.HTTP_204_NO_CONTENT)
    ...
    def post(self,request,*args,**kwargs):
        ...
        try:
            ...
            if user.is_staff and user.is_admin:
                return Response({ 'Message': f"{user.email} is already Promoted to admin"}, status=200)
            user.is_staff = True
            user.is_admin = True
            user.save()
...
class ListAdminView(mixins.RetrieveModelMixin,
                    mixins.UpdateModelMixin,
                    mixins.ListModelMixin,
                    viewsets.GenericViewSet):
    '''list all admins'''
